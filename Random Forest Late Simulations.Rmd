...
This document explores Causal Random Forests Ability to detect heterogeneous treatment effects with different signs - i.e. the ability to detect defiers in a first stage regression equation.

We start by defining some simulation functions that will be useful:
```{r}
library(dplyr)
library(ggplot2)
library(tidyr)
...
many_tau_pvals <- many_tau_pvals %>% 
  mutate(two_pval = 2*pnorm(-abs(t_stat)),
...
many_tau_pvals %>% 
  group_by(draw) %>% 
  summarise(reject_H0_pct = mean(reject_H0_twosided))
```
...
many_tau_pvals %>% 
  group_by(draw) %>% 
  summarise(rejected_at_least_once = ifelse(sum(reject_H0_twosided), 1, 0))
```
...
many_tau_pvals %>% 
  ...
  summarise(rejected_at_least_once = ifelse(sum(reject_H0_twosided), 1, 0)) %>% 
  ungroup() %>% 
  summarise(rejected_H0_once_pct = mean(rejected_at_least_once))
```
