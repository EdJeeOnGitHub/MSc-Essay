...
perform_inference_mono <- function(tau_hat_data, alpha, n_covariates){
  ...
  tau_transformed <- tau_hat_data %>% 
    mutate(t_stat = predictions / sqrt(variance.estimates),
           ...
           p_val = pt(t_stat, df = n - n_covariates - 2),
           p_val_adjusted = p.adjust(p_val, method = "bonferroni"),
           reject_H0_adjusted = ifelse(p_val_adjusted < 0.05, 1, 0))
...
find_number_of_defiers <- function(tau_hat_t_stat, tau){
  ...
  n_significant_defiers <- sum(tau_hat_t_stat$reject_H0_adjusted)
...
# tictoc::toc()
# beepr::beep()
# write.csv(defier_beta_binary_taus, file = "beta_binary_taus.csv", row.names = FALSE)
...
# tictoc::toc()
# beepr::beep()
# write.csv(defier_prop_binary_taus, file = "defier_prop_binary_taus.csv", row.names = FALSE)
...
Now Benjamini Hochberg correction for the false discovery rate:
```{r}
corrected_tau_p_vals <- many_tau_pvals %>% 
  group_by(draw) %>% 
  mutate(one_pval_adjusted = p.adjust(one_pval, method = "bonferroni"),
         two_pval_adjusted = p.adjust(two_pval, method = "bonferroni"))
corrected_tau_p_vals %>% 
  filter(draw <= 20) %>% 
  ggplot(aes(x = one_pval_adjusted)) +
  geom_histogram() +
  facet_wrap(~draw)
```
...
corrected_tau_p_vals %>% 
  group_by(draw) %>% 
  summarise(rejected_H0 = sum(ifelse(one_pval_adjusted < 0.05, 1, 0)))
...
corrected_tau_p_vals %>% 
  group_by(draw) %>% 
  summarise(rejected_H0_one = sum(ifelse(one_pval_adjusted < 0.05, 1, 0)),
            rejected_H0_two = sum(ifelse(two_pval_adjusted < 0.05, 1, 0))) %>% 
  ungroup() %>% 
  summarise(FDR_one = sum(ifelse(rejected_H0_one > 0, 1, 0))/ length(rejected_H0_one),
            FDR_two = sum(ifelse(rejected_H0_two > 0, 1, 0)) / length(rejected_H0_two))
```
