...
Creating data, treatment effect of instrument now varies continuously (actually a normal r.v. with mean 1 and sd 2):
```{r}
rm(list = ls())
create_sim_data <- function(n = 1000, n_variables = 10, binary = TRUE, prop_treated = 0.5, error_sd = 1,Z_FUN = runif, z_a = NULL, z_b = NULL, tau_FUN = rnorm, ...){
  X <- matrix(rnorm(n*n_variables), n, n_variables)
...
library(modelr)
library(purrr)
binary_boots <- continuous_fake_df %>%
  select(-tau) %>% 
  bootstrap(n = 4000)
ols_on_boot <- function(strap){
  lm(Y ~ . , data = strap)
}
binary_boot_models <- binary_boots %>%
  mutate(model = map(strap, ols_on_boot),
         coef_info = map(model, tidy))

binary_boot_coefs <- binary_boot_models %>%
  unnest(coef_info)
...
```{r}
tau_boot_coefs <- binary_boot_coefs %>%
  filter(term == "Z") %>%
