...
abstract: I present some practical tests for detecting failure of the monotonicity
  assumption essential for causal identification under the LATE theorem. Simulations
  show impressive results for the saturated first stage and Random Forest methods.
  I apply these tests to a sample of papers in labour economics using instrumental
...
There's no evidence of defiance in @Angrist_Lavy's estimates of class size and grades and with 2000 observations we can be reasonably confident of test power - indeed, in this range we'd expect the saturated first test to excel in comparison to its machine learning cousin.
...
Meanwhile, @Autor_Dorn_Hanson's identification strategy may be suspect - almost 7\% of the dataset are estimated to be defiers and we reject the null of no defiance at 0.1\% significance. 
...
```{r all_results, results="asis"}

# all_results <- inner_join(first_stage_summary,
#                           tau_hat_summary,
#                           by = c("paper", "dataset"),
#                           suffix = c("_first_stage", "_forest"))
#
