...
simulations_size %>% 
  ...
  summarise("Number of Test Draws" = nrow(simulations_size)/2,
            "Number of Significant Tests" = n(),
            "Proportion Significant" = `Number of Significant Tests` / `Number of Test Draws`,
            "Alpha" = 0.05) %>% 
  gt() %>% 
  fmt_percent(
    columns = c("Proportion Significant",
                "Alpha")
  ) %>% 
  tab_header(
    title = "Size Simulations"
  )
...
It's difficult to implement Romano and Wolf's resampling multiple adjustment procedure with one-sided tests in R - instead I offer suggestive evidence that the procedure is too conservative when considering one draw of the data generated under a two-sided null hypothesis. Full proof via simulation would require simulating $N$ datasets and resampling each dataset $n$ times to form Romano-Wolf adjusted critical values which is omitted in this essay for computational reasons.


```{r romano_wolf_sims}

positive_sim_data <- create_fake_data(N = 1000,
                 model_betas = rep(0, 10),
                 force_positive = TRUE)

positive_sim_model <- positive_sim_data %>%
  select_at(vars(-contains(":"), -`(Intercept)`)) %>% 
  run_first_stage_interactions_fast(.,
                                    "D",
                                    "Z")
...
model_boot <- 1:500 %>% 
  future_map_dfc(.,
                 anon_function,
                 data = positive_sim_data,
                 .progress = FALSE,
                 .options = future_options(
                   globals = c("positive_sim_data",
                               "run_first_stage_interactions_fast",
                               "find_SEs"),
                   packages = c("dplyr",
                                "modelr",
  ...
  as.matrix()
plan(sequential)
adjusted_val <- FWERkControl(test_stat = positive_sim_model$t_stat,
...
positive_sim_model_rf <- sim_first_stage_forest(positive_sim_data)


anon_function_rf <- function(dummy_arg,
                             data){
    data %>% 
    resample_bootstrap() %>% 
    as.data.frame() %>% 
...
if (run_sim){
  ...
  plan(sequential)
  # write.csv(model_boot_rf, "C:/Users/edjee/Dropbox/Ed/romano_wolf_rf.csv",
  #           row.names = FALSE)
} else {
  model_boot_rf <- readr::read_csv("C:/Users/edjee/Dropbox/Ed/romano_wolf_rf.csv") %>% 
    as.matrix()
}

adjusted_val_rf <- FWERkControl(test_stat = positive_sim_model_rf$t_stat,
                             boot_stat = model_boot_rf,
                             k = 1,
                             alpha = 0.05)
...
rf_row <- bind_cols(
  adjusted_val_rf$CV %>% 
    enframe() %>% 
    select("Adjusted Critical Value" = value),
  positive_sim_model_rf %>% 
    summarise("Original Critical Value" = qt(0.95, df = nrow(positive_sim_data) - 10),
              "Max Test Statistic" = max(t_stat),
              "Proportion Significant" = sum(ifelse(t_stat > 1.65, 1, 0))/nrow(positive_sim_data),
              "Test" = "Causal Forest")
)
...
sfs_row <- bind_cols(
  ...
  summarise("Original Critical Value" = qt(0.95, df = nrow(positive_sim_data) - 10),
            "Max Test Statistic" = max(t_stat),
            "Proportion Significant" = sum(ifelse(t_stat > 1.65, 1, 0))/nrow(positive_sim_data),
            "Test" = "Saturated First Stage"))


bind_rows(sfs_row,
          rf_row) %>% 
  gt() %>% 
  fmt_number(columns = c("Adjusted Critical Value",
                         "Original Critical Value",
                         "Max Test Statistic"),
             decimals = 2) %>% 
  fmt_percent(
    columns = "Proportion Significant"
  ) %>%
    tab_header(
    title = "Romano-Wolf Adjustment"
  ) 
