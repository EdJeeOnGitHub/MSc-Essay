...
The difference between the figure above and its predecessor clearly motivates the use of a log transform. It's worth noting that each y-axis is on a different scale, on the larger dataset the tests produce $p$-values orders of magnitude smaller than the medium and small dataset.

Even at low levels of defiers the tests perform well, only the smallest dataset struggles to identify defiers.  
...
In the figure below we can see that as mean partial effect moves away from 0, for both positive and negative first stages, it gets harder and harder to detect defiers.
...
Power increases as defier proportion increases
